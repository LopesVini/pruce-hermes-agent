...
- `profile`: a partial update containing `introduced`, `context` and/or a
  structured `profile` patch. `context` still replaces the previous free-text
  string, so preserve it unless the owner actually corrected or added a durable
  fact. Structured fields merge independently and never rewrite `context`.
  `read` and `active` return `onboarding.complete`, `onboarding.legacy` and the
  remaining profile fields. A legacy `introduced: true` state without a
  structured profile is complete and is not rewritten during a read.
- Structured profile values use explicit status rather than prose memory:
  `preferred_name`, `university` and `course` are
  `{"status":"pending|known|skipped","value":...}`; timezone is
  `{"status":"pending|known|unknown|skipped","value":"<IANA or null>","city":"<city or null>"}`;
  `primary_radar_preference` is `pending`, `university_deadlines`,
  `important_replies`, `adulting_bureaucracy` or `skipped`. Sources are separate
  and never block profile completion.
...
For “Aquilo resolveu?”, “A empresa respondeu?” or “O cancelamento terminou?”,
read the matching current open loop once and reuse that record. If the question
depends on a current reply or provider status, inspect only that authoritative
read-only source. Update the same task ID; never create a second record for the
check.

An action performed is not an outcome resolved. A sent application, cancellation
request, refund request or professor message stays `waiting_for_third_party`
with fresh evidence of the action and the named awaited response. Mark it
`completed` only when current evidence confirms the outcome named by the title,
or when the owner explicitly stops tracking it. An acknowledgement such as
“request received” proves the wait, not completion.

Existing operation receipts and their retry blocks remain authoritative history.
Ordinary reads, drafts, student work and task updates must not load or create
them. Use `pruce-operations` only to reconcile an existing receipt, or in a
