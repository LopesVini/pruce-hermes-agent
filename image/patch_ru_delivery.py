...
REPLACEMENT = ('    if wrap_response and not (not for_failure and _pruce_ru_menu_job(job)):\n'
               '        task_name = job.get("name", job["id"])\n')
HELPER = '''
def _pruce_ru_menu_job(job: dict) -> bool:
    """Only Prucê's opted-in, script-only RU jobs get a clean delivery body."""
    import json
    import re
